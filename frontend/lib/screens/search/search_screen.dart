...
class _SearchScreenState extends ConsumerState<SearchScreen> {
    ...
    return Scaffold(
      ...
      body: _query.length < 2
          ...
          : searchAsync.when(
              data: (data) {
                if (data.isEmpty ||
                    (data['students']?.isEmpty ?? true) &&
                        (data['faculty']?.isEmpty ?? true) &&
                        (data['events']?.isEmpty ?? true) &&
                        (data['news']?.isEmpty ?? true) &&
                        (data['buses']?.isEmpty ?? true)) {
                  return Center(
                      ...
                      _buildSection(
                          ...
                          (item) => ListTile(
                                leading: CircleAvatar(
                                    backgroundImage: item['profilePicURI'] !=
                                            null
                                        ? NetworkImage(item['profilePicURI'])
                                        : null),
                                title: Text(item['name']),
                                subtitle: Text(
                                    "${item['branch']} • ${item['rollNumber'] ?? ''}"),
                              )),
                    if (data['faculty'] != null && data['faculty'].isNotEmpty)
                      _buildSection(
                          ...
                          (item) => ListTile(
                                ...
                                title: Text(item['name']),
                                subtitle: Text(
                                    "${item['department']} • ${item['cabinNumber'] ?? ''}"),
                              )),
  ...
  Widget _buildSection(
      String title, List items, Widget Function(dynamic) itemBuilder) {
    return Column(
