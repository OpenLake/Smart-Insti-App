...
final confessionProvider =
    StateNotifierProvider<ConfessionNotifier, ConfessionState>((ref) {
...
class ConfessionNotifier extends StateNotifier<ConfessionState> {
  ...
  Future<void> loadConfessions({bool refresh = false}) async {
    if (refresh) {
      state = state
          .copyWith(isLoading: true, page: 1, hasMore: true, confessions: []);
    }
    ...
    try {
      ...
      if (token != null) {
        final newConfessions =
            await _repository.getConfessions(token, page: state.page);
  ...
  Future<bool> createConfession(String content, String backgroundColor) async {
    ...
    if (token != null) {
      final success =
          await _repository.createConfession(content, backgroundColor, token);
      if (success) {
